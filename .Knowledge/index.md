...
| pp-d2c-prep-plugin | `.Knowledge/topics/pp-d2c-prep-plugin.md` | D2C 上游几何预处理 Figma 插件:三阶段迭代合并(O 相交 → A 图形碎片 → B 混文字) + 清理隐藏 / Slice + 一键拆分 + AI 建议;不做前缀自动推断也不反推 Autolayout,几何交给算法、语义交给人 / AI | stock:[pp-d2c-prep-plugin 终稿](.Knowledge/stock-docs/pp-d2c-prep-plugin_终稿.md);源码:`plugin/src/analyzer/mergeIterative.ts`;关联:[[pp-d2c]] / [[ai-proxy]] |
| ai-proxy | `.Knowledge/topics/ai-proxy.md` | 本地 AI 请求中转(端口 8787),把 Figma 插件调用转发到内网 PETA `gpt-6-astra`;两版并存(Node 主用 + Python 兜底);硬编码 `temperature=1` / `stream=true`;处理公司自签 CA、SSE 拆包、30MB body 上限 | stock:[ai-proxy 终稿](.Knowledge/stock-docs/ai-proxy_终稿.md);源码:`ai-proxy-node/server.mjs` / `ai-proxy/main.py`;关联:[[pp-d2c-prep-plugin]] |
