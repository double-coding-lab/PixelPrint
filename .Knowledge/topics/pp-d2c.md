...
二者独立递增：改一条 skill 规则会 bump SKILL 版本但未必 bump npm；发一次 npm 可能只动 `install.js` 而 SKILL 版本不变。**看到 SKILL `v1.2.4` 与 npm `1.3.1` 不一致属预期，不是 bug**。（pp-d2c-rn `v0.4`、pp-d2c-fast 等各 skill 的 SKILL 版本同样与 npm 独立。）
