...
The repository-root **[`AGENTS.md`](../AGENTS.md)** is the complete Flow2Spec project guide. Codex [auto-loads](https://developers.openai.com/codex/guides/agents-md) it when started from the repository root.
...
| `hooks.json` | Codex SessionStart hook configuration for checking the Flow2Spec knowledge-base version on startup |
| `hooks/` | Hook script directory |
| `config.toml` | Project-level Codex configuration, if created |

Configuration source of truth: repository-root **`flow2spec.config.json`** (must be Read); the init-time snapshot table is in root **`AGENTS.md`**.
