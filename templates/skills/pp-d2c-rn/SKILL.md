...
- **产物**:`<assetsDir>/<slug>/*.png` + `slice-manifest-<slug>.json`,后续 §4.4 图片处理(见契约 §2)由 sub-agent 只读清单消费,不再自调 Figma API
- **RN 侧 sub-agent 引用形式**:`require('...png')` / `source={{uri}}` / `<ImageBackground>` / `<FastImage>` / `` `${ASSET_PREFIX}xxx.png` ``——与 IMG-reconcile 对账口径一致
- **三层去重**(命名冲突消解 / list- 同构 / **内容 md5 兜底**)、切图确认暂停、`confirmed` 留痕 → 契约 §1.3 / §1.8(rn 侧 confirm-slices 走 `pp-d2c-rn/bin/figma.mjs`)
...
>
> **sub-agent QA 自证格式**（`blocks/{sub}/assets.txt` QA 段末尾追加）：
>
> ```
> ## bg-* 独立切图清单证明（v0.3.11）
>
...
> ```
>
> **主 agent 合并前 grep 自证命令**（§6.0.3 证明块「bg- 独立切图契约」段引用，RN 侧覆盖 5 种 Image 引用形式）：
>
> ```bash
> # 1. 从子树 JSON 提取所有 bg-* 节点名（不含 bgc-）
> grep -Eho '"name":\s*"(bg-[^"]+|bg)"' .d2c-cache/**/nodes/*.json 2>/dev/null \
>   | sed -E 's/.*"name":\s*"([^"]+)".*/\1/' | sort -u > /tmp/rn-bg-nodes.txt
>
> # 2. 从 assets.txt 提取已切图的 bg-* 文件名
> grep -Eho '^- +bg-?[A-Za-z0-9_-]+\.(png|svg|jpg|jpeg|webp)' {output.dir}/blocks/**/assets.txt 2>/dev/null \
>   | sed 's/^- *//' | sed -E 's/\.(png|svg|jpg|jpeg|webp)$//' | sort -u > /tmp/rn-bg-declared.txt
>
> # 3. 差集
> MISSING=$(comm -23 /tmp/rn-bg-nodes.txt /tmp/rn-bg-declared.txt | wc -l)
>
> # 4. 产物侧覆盖 RN 5 种 Image 引用形式（require / uri / ImageBackground / FastImage / ASSET_PREFIX）
> #    验证每个已切 bg-* 都能在产物中找到至少一处引用
> for bg in $(cat /tmp/rn-bg-declared.txt); do
>   USED=$(grep -rEo "require\(['\"]\S*${bg}\.png['\"]\)|source=\{\{uri:.*${bg}\.png|<ImageBackground[^>]*${bg}|<FastImage[^>]*${bg}|\\\$\{ASSET_PREFIX\}${bg}\.png" \
>     {output.dir}/ --include='*.tsx' --include='*.jsx' --include='*.ts' --include='*.js' 2>/dev/null | wc -l)
>   if [ "$USED" = "0" ]; then
>     echo "⚠️  bg-* 切了但产物没引用：$bg（RN 5 种 Image 形式全匹配 0 次）"
>   fi
> done
>
> if [ "$MISSING" = "0" ]; then
>   echo "✅ RN bg- 独立切图契约通过：$(wc -l < /tmp/rn-bg-nodes.txt) 个 bg-* 节点全部独立切图"
> else
>   echo "❌ $MISSING 个 bg-* 节点未独立切图，可能被祖先切图省略（触发 doctor BGP033）"
>   comm -23 /tmp/rn-bg-nodes.txt /tmp/rn-bg-declared.txt
> fi
> ```
>
> **典型案例**：某父容器 `bg-<A>` 整体切了 `<A-bg>.png`，agent 因为该切图物理覆盖了下方多个同级列表项所在区域，就"省略"了每个列表项里 `bg-<B>` 的独立切图，产物对应容器空 View、后代装饰完全丢失。
>
> **doctor 关联规则**：BGP033（v0.3.11 新增，error），见 pp-doctor §3.6v。
...
> **底层切图规则以** [`../pp-d2c-reskin/SLICE-CONTRACT.md`](../pp-d2c-reskin/SLICE-CONTRACT.md) **§2 图片处理契约 + §3 Token 处理 + §4 切图适格性 + §5 bg- 独立切图为准**(三 skill 共享的切图契约主本;RN 特有的 §4.4.pre / §4.4.pre.b / bg- 独立切图段已合并到契约 §4 / §5,统一维护)。本 SKILL 只保留**门禁短句** + **RN 侧产物代码规则**(require 编译期路径,与切图规则正交)。

**门禁短句(必读,不得删)**:

- **忠实执行**:命中 `img-` / `bg-` / 裸词 `img` / 裸词 `bg` 时,**必须**调 `figma.mjs export-image`(REST API 单一路径),**禁止**"看到 assetsDir 里有同名文件就跳过"或"从其他来源复用" → 契约 §2.1
- **补切场景收口**:v1.1.0 起 `img-` / `bg-` 前缀节点由步骤 2.6 前置切图统一导出,sub-agent 只查 `slice-manifest` 消费 `filename`;**sub-agent 禁止自调 `figma.mjs export-image`**。`figma.mjs export-image` 直切仅存两种场景:① 主 agent 补切回路;② 无前缀兜底 fills IMAGE / `input-` 图标等 2.6 扫描范围外的零星导出——均由主 agent 执行并回写 manifest,否则 IMG-reconcile 按"绕清单切图"报 violation
- **前置自检**:调 `figma.mjs export-image` 前,切图执行方每张图**必须**输出 7 行(图层前缀类型 / 切图源 nodeId / 切图源 name / 交叉验证 / 切图范围 / 子树可拆分子节点数 / 结构维度禁切判定)——普通 3 行通用 + RN 特有 4 行适格性核查,命中"意图切父容器"或"结构禁切"立即停下重做 → 契约 §2.2
- **适格性核查(RN 特有,判定优先于前缀)**:即使前缀允许整体切图(`img-` / `bg-` / 无前缀),只要子树命中"≥2 可见 TEXT 分属不同视觉行" / "≥2 btn/INSTANCE/COMPONENT" / "≥3 同层同构"任一,一律禁止整体切图,必须递归子层 → 契约 §4.2;违反后果 doctor SUB027(前缀维度) / SUB029(结构维度),见 pp-doctor §3.6p / §3.6r
...
- **Token 过期**(HTTP 401 / 403):立即终止,输出错误提示由用户补 token 后重跑,**禁止**用临时链接占位 / 调用任何 `mcp__plugin_figma_figma__*` / 用 MCP `download_assets` 兜底(不支持 `use_absolute_bounds=true`,会重现 `card-bg.png` 类历史 bug) → 契约 §3

**doctor IMG026**:`img-` / `bg-` / 裸词 img/bg 命中,但 images.json 里对应 nodeId 缺失 → **error**(说明本次 skill 没走 REST 就落图,属于严重忠实度事故)。

**历史事故取证**:某轮 `<container>-<item>-<state>.png` 出现"顶部有大片空白",追查 md5 发现磁盘产物和 API 实测导出根本对不上——skill 那一轮实际上没调 REST,而是从上一轮的另一张同类图直接改名复用了。切图忠实度门禁就是为这类事故立的。
...
> 判定规则见 [`../pp-d2c-reskin/SLICE-CONTRACT.md`](../pp-d2c-reskin/SLICE-CONTRACT.md) §4;本段是合并阶段的汇报格式模板。
...
> 判定规则见 [`../pp-d2c-reskin/SLICE-CONTRACT.md`](../pp-d2c-reskin/SLICE-CONTRACT.md) §5;本段是合并阶段的汇报格式模板。
