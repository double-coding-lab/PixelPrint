...
8. Use `manual` only when the user or maintainer explicitly confirms the category value. If there is clear evidence but no human confirmation of the category value, write `inferred`. When evidence is insufficient, **do not write metadata**, but list the inferred direction and evidence in the summary (for example, "suggest policy; the body contains multiple mandatory constraints") so the user can confirm and manually write `manual`. **Do not infer classification only from the topicId name; Read the topic body before deciding.**
